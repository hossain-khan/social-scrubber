...
class ScrubConfig(BaseModel):
    ...
    start_date: str = Field(default="7_days_ago", description="Start date for scrubbing")
    end_date: str = Field(default="today", description="End date for scrubbing")
    max_posts_per_scrub: int = Field(default=10, description="Maximum posts to scrub per run")
    dry_run: bool = Field(default=True, description="Whether to run in dry-run mode")
    archive_before_delete: bool = Field(default=True, description="Archive posts before deletion")
    archive_path: str = Field(default="./archives", description="Path to store archives")
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            scrub=ScrubConfig(
                ...
                archive_before_delete=os.getenv("ARCHIVE_BEFORE_DELETE", "true").lower() == "true",
