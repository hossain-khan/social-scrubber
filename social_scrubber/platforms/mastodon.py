...
class MastodonPlatform(BasePlatform):
    ...
    async def authenticate(self) -> bool:
        ...
        if not self.config.is_configured:
            print("❌ Mastodon configuration missing. Please check MASTODON_API_BASE_URL and MASTODON_ACCESS_TOKEN.")
            return False
        ...
        try:
            ...
            print(f"✅ Successfully authenticated with Mastodon as @{account['username']}")
            return True
    ...
    async def delete_post(self, post_id: str) -> DeletionResult:
        ...
        if not self._authenticated or not self.client:
            return DeletionResult(
                post_id=post_id,
                success=False,
                error="Not authenticated with Mastodon"
            )
