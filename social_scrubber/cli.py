...
from .utils import (
    setup_logging, print_banner, print_platform_status, 
    display_posts_table, display_deletion_results, 
    confirm_action, format_date_range, ensure_archive_directory
)
...
class SocialScrubber:
    ...
    async def authenticate_platforms(self, selected_platforms: Optional[List[str]] = None) -> Dict[str, bool]:
        ...
        platforms_to_auth = (
            selected_platforms if selected_platforms 
            else list(self.platforms.keys())
        )
    ...
    async def get_posts_from_platforms(
        ...
        for platform_name in platform_names:
            ...
            try:
                ...
                if posts:
                    console.print(f"✅ Found {len(posts)} posts from {platform.display_name}")
                else:
                    console.print(f"ℹ️ No posts found from {platform.display_name} in date range")
            
            except Exception as e:
                console.print(f"❌ Error fetching posts from {platform.display_name}: {e}")
    ...
    async def delete_posts_from_platform(
        ...
        if dry_run:
            console.print(f"🔍 [DRY RUN] Would delete {len(posts)} posts from {platform.display_name}")
            return []
        ...
        if self.config.scrub.archive_before_delete:
            if not ensure_archive_directory(self.config.scrub.archive_path):
                console.print("❌ Failed to create archive directory. Aborting deletion.")
                return []
    ...
    async def run_interactive(self):
        ...
        console.print(f"🔢 Max posts per platform: {self.config.scrub.max_posts_per_scrub}")
        console.print(f"🧪 Dry run mode: {'ON' if self.config.scrub.dry_run else 'OFF'}")
        ...
        for platform_name, posts in all_posts.items():
            ...
            results = await self.delete_posts_from_platform(
                platform_name, 
                posts, 
                dry_run=self.config.scrub.dry_run
            )
